...
### SAR·EO 페이지 DB 연동 (satellite_intel)

두 페이지 모두 같은 방식으로 `satellite_intel` 스키마와 연동됩니다.
(SAR은 SQL이 `features/sar/repository.py`에, EO는 `features/eo/view.py` 안에 있습니다 — 담당자별 스타일 차이)

- **업로드 파일명이 곧 메타데이터**입니다. 형식: `자산명_지역명_지역ID_센서_YYYY-MM-DD HHMMSS.확장자`
  - 예: `425-1_개풍군_1_SAR_2026-07-09 100000.tif`, `425-1_개풍군_1_EO_2023-12-30 220000.png`
  - 파일명에 `:`를 쓸 수 없어 시각은 붙여 씁니다 (`100000` = 10:00:00)
  - 자산명/지역명에 `_`가 들어가면 파싱에 실패하니 이름에 언더스코어를 쓰지 마세요
- **"DB 저장" 버튼** 하나로 다음을 한 번에 저장합니다:
  - `image_analysis`에 **새 행** INSERT — image_id는 DB가 자동 증가로 부여
  - `detection_result`에 클래스별 집계 (SAR은 편집 반영된 최종 검출목록 기준) — `created_at` = 버튼을 누른 시스템 시간, `avg_confidence` = 미사용 방침이라 0
  - 원본 이미지 → `original_image/`, 박스 그려진 결과 이미지 → `result_image/` (git 미포함)
  - 두 테이블 저장은 하나의 트랜잭션이라 중간에 실패하면 함께 롤백됩니다
- **같은 파일을 다시 저장하면 새 image_id로 행이 추가**됩니다 (화면에 "이미 저장됨" 안내 표시).
...
- SAR의 방위각 자동 회전(`90azi` 파일명 패턴)은 새 파일명 규약에 방위각 자리가 없어 사실상 쓰이지 않습니다 — 회전은 수동 슬라이더로 지정하세요.
