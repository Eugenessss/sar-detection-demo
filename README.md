...
두 페이지 모두 `shared/image_store.py`의 공용 저장 로직으로 `satellite_intel` 스키마와 연동됩니다.
...
- **"DB 저장" 버튼** 하나로 다음을 한 번에 저장합니다:
  - `image_analysis`에 영상 메타데이터 저장 — 처음 보는 영상은 새 행을 만들고, 같은 `(자산명, 지역명, 촬영시각, 센서)` 영상은 기존 `image_id`를 재사용
...
- **같은 파일을 다시 저장하면 새 image_id를 만들지 않고 기존 행의 탐지 결과를 덮어씁니다.**
