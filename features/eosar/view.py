...
def render_controls() -> EosarControls:
    ...
    """
    # ① 데이터 및 이미지 선택 카드
    with bracket_panel("eosar_controls_panel"):
...
def render_daily_checklist_bar() -> None:
    ...
    with bracket_panel("eosar_checklist_panel"):
...
def render_db_save_section(result: InferenceResult, meta: Optional[Dict[str, Any]]) -> None:
    ...
    with bracket_panel("eosar_db_save_panel"):
        st.subheader("DB 저장")
...
def render_report_section(meta: Optional[Dict[str, Any]]) -> None:
    ...
    """
    with bracket_panel("eosar_report_panel"):
        st.subheader("보고서")
...
def render_image_panel(result: InferenceResult, selected_indices: List[int]) -> None:
    ...
    with bracket_panel("eosar_image_panel"):
        st.subheader("탐지 결과")
...
def render_placeholder_panel() -> None:
    ...
    with bracket_panel("eosar_placeholder_panel"):
        st.subheader("탐지 결과")
...
def _render_original_preview(filename: Optional[str], s3_key: str) -> None:
    ...
    PIL로 열어서 넘기므로 문제없다.
    """
    with bracket_panel("eosar_original_preview_panel"):
        st.subheader("탐지 결과")
